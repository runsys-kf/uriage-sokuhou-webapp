...
import { fetchData, API_ENDPOINTS } from "@/app/app1/api/apiService";
...
const AdminPage = () => {
	...
	const navigateToEditPage = (store: StoreInfo) => {
		...
		router.push(`/app1/admin/settings?${query}`);
	};
	...
	const handleLogout = async () => {
		try {
			router.replace("/app1/admin/admin_login"); // pushではなくreplaceを使用
		} catch (error) {
			console.error("Logout failed:", error);
			router.replace("/app1/admin/admin_login");
		}
	};
							...
							<div className="p-2 md:p-4 w-full md:mt-0 mt-14">
								...
								<div className="flex flex-wrap md:flex-row md-2 md:mb-4 gap-2 justify-end">
									<Button
										...
										onClick={() => router.push("/app1/admin/add_settings")}
									>
										新規店舗を追加
									</Button>
								</div>
