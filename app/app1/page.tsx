...
const IndexPage = () => {
  ...
  const [selectedStores, setSelectedStores] = useState<Store[]>([]); //選択した店舗
  const [authorizedStores, setAuthorizedStores] = useState<Store[]>([]); //表示できる店舗
  const [filteredStores, setFilteredStores] = useState<Store[]>([]); //絞り込み店舗
  ...
  const handleSelectOpen = () => {
    ...
    let filtered: Store[] = authorizedStores;
    if (searchText) {
  ...
  // 都道府県のリストを抽出する関数
  const extractPrefecturesFromAreas = (areas: Record<string, string[]>): string[] => {
